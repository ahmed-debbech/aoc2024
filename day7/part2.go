	...
	for _, eq := range eqs{
		...
		for i:=0;i<=int(possible_combs)-1; i++{
			...
			for j:=0; j<=len(final_arr_op)-1; j++{
				...
				if final_arr_op[j] == '|' {
					fg := strconv.Itoa(int(s))
					fh := strconv.Itoa(int(eq.Vals[k]))
					fg += fh
					x, _ := strconv.Atoi(fg)
					s = int64(x)
				}
